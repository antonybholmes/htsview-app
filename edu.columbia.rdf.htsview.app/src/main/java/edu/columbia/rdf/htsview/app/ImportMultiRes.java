...
public class ImportMultiRes {
	...
	private static final Logger LOG = 
			LoggerFactory.getLogger(ImportMultiRes.class);
	...
	public static class EncodeWorker extends SwingWorker<Void, Void> {
		 ...
		 */
		public EncodeWorker(ModernWindow parent,
				Path samFile, 
				Path dir,
				String name,
				String organism,
	...
	public static void encodeMultiResFile(Path samFile,
			...
			String genome,
			int readLength,
			int readCount) throws IOException, ParseException {
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				if (c == null || chr == null) {
					System.err.print(c + " " + tokens.get(2) + " " + chr);
				}
	...
	public static int encodeMultiResFileOrdered(Path samFile,
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				c = Chromosome.parse(tokens.get(2));
	...
	public static void encodeMultiResFile(Path samFile,
		...
		while (!blockQueue.isEmpty()) {
			...
			//System.err.println("new bin " + binWidth + " " + bs + " start:" + child.start + " " + MAX_BLOCK_WIDTH);
			...
			if (binWidth <= MAX_BLOCK_WIDTH) {
				...
				//System.err.println("add child " + bs + " " + binWidth + " " + child.start + " " + child.width);
			}
		}
			...
			while (!groupBlockQueue.isEmpty()) {
				...
				for (Block block : blocks) {
					...
					if (block.start == 199999000) {
						System.err.println("write block " + block.start + " " + block.width + " " + block.children.size() + " " + fo);
						System.err.println("write block2 " + dataStartOffset + " " + block.endOffset + " " + (dataStartOffset + block.endOffset * READ_START_BYTES));
					}
