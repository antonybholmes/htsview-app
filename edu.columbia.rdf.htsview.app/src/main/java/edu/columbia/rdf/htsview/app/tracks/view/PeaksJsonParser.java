...
public class PeaksJsonParser extends TrackJsonParser {
  ...
  public boolean parse(ModernWindow window, final String name, int id, ModernTree<Track> annotationTree,
    ...
    List<GenomicRegion> locations = WebAssemblyService.getInstance().getPeakAssembly().downloadJsonPeaks(id, peaks);

    Bed bed = Bed.create(peaks.getName(), locations);
    bed.setColor(color);

    rootNode.addChild(new TreeNode<Track>(peaks.getName(), new PeaksPlotTrack(id, peaks, bed)));
