  ...
  public void aaPlot(Graphics2D g2,
    ...
    for (Entry<String, Set<GenomicElement>> e : mGeneCache) {
      ...
      for (GenomicElement g : mGeneCache.get(symbol)) {
        ...
        for (GenomicElement exon : g.getChildren(GenomicType.UTR_5P)) {
        ...
        for (GenomicElement exon : g.getChildren(GenomicType.UTR_3P)) {
          x1 = axes.toPlotX1(exon.mStart);
          x2 = axes.toPlotX1(exon.mEnd);
        ...
        for (GenomicElement exon : g.getChildren(GenomicType.EXON)) {
          x1 = axes.toPlotX1(exon.mStart);
          x2 = axes.toPlotX1(exon.mEnd);
