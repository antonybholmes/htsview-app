...
public class GenesPlotLayer extends AxesLayer {
	...
	public void aaPlot(Graphics2D g2,
		...
		for (String symbol : mGeneCache.keySet()) {
			...
			//		(mGenes.findMainVariant(g.getSymbol()) != null && mGenes.findMainVariant(g.getSymbol()).equals(g));

			//g2.setFont(isMainVariant ? mGeneProperties.getVariantGene().getFont().getFont() : 
			//	mGeneProperties.getOtherGene().getFont().getFont());

			//g2.setColor(isMainVariant ? mGeneProperties.getVariantGene().getFont().getColor() : 
