...
public class ImportDialog extends ModernDialogTaskWindow {
	...
	/**
	 * Loads a list of the genomes in use in the chr database since this
	 * is what a user can use for mapping purposes.
	 * 
	 ...
	 * The m field name.
	 */
	private InputFieldBox mFieldName = 
			new InputFieldBox("Name", 100, 300);
	
	/**
	 * The m field organism.
	 */
	private InputFieldBox mFieldOrganism = 
			new InputFieldBox("Organism", "Homo Sapiens", 100, 300);
	
	/**
	 * The m field read length.
	 */
	private InputNumericalFieldBox mFieldReadLength = 
			new InputNumericalFieldBox("Read Length", 101, 100, 50);
	...
	public ImportDialog(ModernWindow parent) {
		...
		mChooseSamFilePanel = 
				new ChooseFilePanel(parent, new SamGuiFileFilter());
